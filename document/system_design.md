...
1. Nhận yêu cầu mới (Webhook hoặc gọi trực tiếp từ frontend với token tạm).
2. Fetch metadata YouTube, xử lý transcript theo chiến lược 2 bước:
   - B1: Thử lấy caption có sẵn của YouTube (bao gồm auto-generated captions). Nếu tồn tại track caption phù hợp ngôn ngữ → dùng trực tiếp làm transcript.
   - B2: Nếu video không có caption/auto-caption → tải audio và chạy qua dịch vụ Speech-to-Text (Whisper/Google STT, ...) để tự sinh transcript.
3. Tính độ khó, phân đoạn transcript → insert `reading_segments`.
4. Sinh quiz nghe, dictation prompt, vocabulary candidates → insert vào bảng tương ứng.
5. Cập nhật `study_sessions.status` = `ready`.
