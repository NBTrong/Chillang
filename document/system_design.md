...
2. Fetch transcript từ RapidAPI YouTube Transcriptor API:
   - Gọi `https://youtube-transcriptor.p.rapidapi.com/transcript?video_id=...` với RapidAPI key.
   - API trả về array, phần tử đầu chứa `transcriptionAsText` (plain text) và `transcription` (array với timestamp).
   - Nếu video không có caption → trả lỗi `"Video này không có caption"` (404).
   - Nếu có → trả `{ transcript, language, isAutoGenerated: null }` (RapidAPI không phân biệt human vs auto-generated) và kèm metadata video (title, thumbnails, length...).
   - Edge Function (chạy bằng service role key) sẽ:
     1. Upsert `videos` (owner_id + youtube_video_id) với metadata + transcript json.
     2. Xoá + insert lại `reading_segments` dựa trên `transcription` (sử dụng timestamp/duration).
